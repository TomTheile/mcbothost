...
function startBot(config) {
    return new Promise((resolve) => {
        ...
        try {
            ...
            // Korrektur: mcVersion kann "1.21" sein, während der Server "1.21.4" erwartet
            // Deshalb sicherstellen, dass wir die vollständige Version verwenden
            const botVersion = mcVersion === '1.21' ? '1.21.4' : (mcVersion || '1.21.4');
            ...
            // Speichern der Verbindungsinformationen für spätere Wiederverbindungen
            ...
            bot.on('kicked', (reason) => {
                ...
                if (!shouldNotReconnect) {
                    ...
                    if (reconnectAttempts[username] <= 10) {
                        console.log(`Versuche, Bot ${username}_Bot automatisch neu zu verbinden... (Versuch ${reconnectAttempts[username]}/10)`);
                        addBotLog(username, `Wiederverbindungsversuch ${reconnectAttempts[username]}/10`, 'warning');
                        ...
                        console.log(`Maximale Anzahl an Wiederverbindungsversuchen (10) erreicht für ${username}_Bot`);
                        addBotLog(username, `Maximale Anzahl an Wiederverbindungsversuchen (10) erreicht. Der Bot bleibt offline.`, 'error');
                    }
                }
            });
            ...
            bot.on('end', () => {
                ...
                if (reconnectAttempts[username] <= 10) {
                    console.log(`Versuche, Bot ${username}_Bot nach Verbindungsabbruch automatisch neu zu verbinden... (Versuch ${reconnectAttempts[username]}/10)`);
                    addBotLog(username, `Wiederverbindungsversuch nach Verbindungsabbruch ${reconnectAttempts[username]}/10`, 'warning');
                    
                    // Server-Adresse und Port aus Connection-Info oder vom Bot holen
                    const serverHost = connectionInfo?.host || (bot.server ? bot.server.host : (bot._client ? bot._client.socket.remoteAddress : host));
                    const serverPort = connectionInfo?.port || (bot.server ? bot.server.port : (bot._client ? bot._client.socket.remotePort : parseInt(port)));
                    // Wichtig: Sicherstellen, dass wir 1.21.4 und nicht 1.21 verwenden
                    let botVersion = connectionInfo?.version || bot.version || config.mcVersion || '1.21.4';
                    if (botVersion === '1.21') botVersion = '1.21.4';
                    const botName = connectionInfo?.botName || bot.username || username + '_Bot';
                    ...
                    console.log(`Maximale Anzahl an Wiederverbindungsversuchen (10) erreicht für ${username}_Bot nach Verbindungsabbruch`);
                    addBotLog(username, `Maximale Anzahl an Wiederverbindungsversuchen (10) erreicht. Der Bot bleibt offline.`, 'error');
                }
            });
